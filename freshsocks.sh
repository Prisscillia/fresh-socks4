...
wget -O ./sockpile/cleansocks "https://hidemy.name/api/checker.php?out=plain&action=export&working&groups=$checkedsocks"

		#Format proxies for ProxyChains.conf 
		sed -i -e 's/^/socks4 /' -e 's/:/ /' ./sockpile/cleansocks

#How do I get the OLD Proxies out of the .conf  ?
#sed -i 116q /etc/proxychains4.conf # WArning this is a personal configuration for MY BOX 
